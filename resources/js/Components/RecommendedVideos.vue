...
<template>
    <div @mouseover="show = true" @mouseleave="show=false">
        <img width="340" class="aspect-video cursor-pointer rounded-lg" :src="vid.thumbnail || '' " :class="show ? 'delay-350 hidden' : ''">
        <div class="w-full h-full aspect-video cursor-pointer" :class="show ? '' : 'delay-350 hidden'">
            <video width="340" :src="video || '' " ref="video" class="rounded-lg cursor-pointer"/>
        </div>
    </div>
    <div class="w-[500px]">
        <div class="flex mb-3">
            <div class="px-1.5 pl-3 text-white mt-1">
                ...
                <p class="text-[12px] text-gray-300 font-extrabold flex gap-1 items-center cursor-pointer">{{ vid.user }}
