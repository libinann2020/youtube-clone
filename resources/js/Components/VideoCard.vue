<template>
    <div class="relative">
        <div :class="[show ? 'absolute z-30 transition ease-in-out delay-150 hover:translate-y-8 hover:scale-125 hover:bg-[#202020] duration-300' : '']" class="rounded-ls bg-black m-2">
            <div @mouseover="show = true" @mouseleave="show=false; showVideo = false">
                <img class="aspect-video cursor-pointer" :src="thumbnail || '' " :class="[show ? 'transition ease-in-out delay-150 rounded-l-lg': 'rounded-l-lg' , showVideo ? 'delay-350 hidden' : '']">
                <div class="w-full h-full aspect-video cursor-pointer" :class="showVideo ? '' : 'delay-350 hidden'">
                    <video :src="videoUrl || '' " ref="video" type="video/mp4"/>
                </div>
            </div>
            <div class="flex mt-1 5">
                ...
                <div class="px-1.5 text-white mt-1">
                    ...
                    <p class="text-[14px] text-gray-300 font-extrabold flex gap-1 items-center cursor-pointer">{{ user.substring(0, 30) }}
